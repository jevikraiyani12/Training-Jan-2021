...
import { IShowSeatPrice } from './IShowSeatPrice';

@Injectable({
  providedIn: 'root'
})
export class ShowSeatPricesService {

  private  apiServer = HostUrl + '/api/ShowSeatPrices'
 // private apiServer = "http://20.198.103.48:1019/api/Screens";
  ...
  constructor(private httpClient: HttpClient) { }
   ...
   getAll(): Observable<IShowSeatPrice[]> {
    return this.httpClient.get<IShowSeatPrice[]>(this.apiServer)
    .pipe(
  ...
  create(data): Observable<IShowSeatPrice> {
    return this.httpClient.post<IShowSeatPrice>(this.apiServer , JSON.stringify(data), this.httpOptions)
    .pipe(
      catchError(this.errorHandler)
    )
  }  
